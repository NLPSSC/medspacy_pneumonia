...
ClassificationResult: TypeAlias = Literal["POS", "NEG", "POSSIBLE"]
...
class BaseDocumentClassifier(ABC):
    ...
    def __call__(self, doc, normalized=False, classification_schema=None, **kwargs):
        doc._.document_classification = self.classify_document(
            doc,
    ...
    def classify_document(
        self, doc, normalized=False, classification_schema=None, **kwargs
    ) -> ClassificationResult:
        classification = self._classify_document(
            doc, classification_schema=classification_schema, **kwargs
        )
        if normalized:
    ...
    def normalize_document_classification(self, classification) -> ClassificationResult:
