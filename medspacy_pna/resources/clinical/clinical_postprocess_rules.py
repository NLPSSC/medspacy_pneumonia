...
def disambiguate_impression(span, section_title, window_size=5):
    ...
    if preceding_idx >= 0:
        ...
        if (
            preceding_token._.section_title.text.lower().startswith("impression")
            and preceding_token._.section_category == "impression"
        ):
            # return
            return disambiguate_impression(
                span, preceding_token._.section_title, window_size=window_size
            )
        if preceding_token._.section_category in (
            ...
            return True
        if preceding_token._.section_title._.contains(
            r"indication|finding|studies", case_insensitive=True, regex=True
        ):
            return True
    # except IndexError:
    #     pass
...
def disambiguate_cap(ent):
    ...
    if ent._.section_category == "medications":
        return False
    if ent.sent._.contains(
        r"mg|medication|capsule|mouth|po|oral|qday|zole|refill",
        regex=True,
...
postprocess_rules = [
    PostprocessingRule(
        patterns=[
            PostprocessingPattern(
                lambda ent: ent._.section_category in ("impression", "addendum")
            ),
            PostprocessingPattern(
                lambda ent: ent._.section_title._.contains(
                    "(diagnos|<<ADDENDUM>>)", regex=True, case_insensitive=True
                )
                is False
            ),
            PostprocessingPattern(
                lambda ent: disambiguate_impression(
                    ent, ent._.section_title, window_size=5
                )
            ),
            # PostprocessingPattern(lambda ent: section_preceding(ent) == "")
        ],
        action=change_ent_section,
        action_args=("imaging",),
    ...
    PostprocessingRule(
        patterns=[
            ...
            PostprocessingPattern(
                lambda ent: postprocessing_functions.is_modified_by_text(ent, r"risk")
            ),
            PostprocessingPattern(
                lambda ent: postprocessing_functions.is_followed_by(
                    ent, target="is low", window=ent.sent.end - ent.end
                )
            ),
        ],
