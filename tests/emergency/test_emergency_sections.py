...
class TestEmergencySections:
    ...
    def test_disambiguate_impression(self):
        ...
        failed = []
        ap_texts = [
            "Diagnostic Impressions: Pneumonia",
            "Impression: Pneumonia",
            "<<ADDENDUM>>: Pneumoia",
        ]
        for text in ap_texts:
            doc = nlp(text)
            section = doc._.sections[-1]  # type: ignore
            try:
                assert section.category != "imaging"
        ...
        failed = []
        imaging_texts = [
            "Labs and Studies: XYZ. Impression: Pneumonia",
            "Procedures: XYZ Impression: Pneumonia",
            "Imaging: XYZ Impression: Pneumonia",
            "Imaging: XYZ Addendum: Pneumonia",
            "Imaging: XYZ Impression: XYZ Addendum: Pneumonia",
    ...
    def test_appended_addendum(self):
        text = "<<ADDENDUM>>:\n\n"
        doc = nlp(text)
        assert doc._.section_categories[0] == "addendum"  # type: ignore
