...
clf = nlp.get_pipe(get_document_classifier_pipe_name(nlp))  # type: ignore

class TestRadiologyLogic:
    def test_link_evidence(self):
        text = "Opacity most likely due to atelectasis."
        doc = nlp(text, disable=[get_document_classifier_pipe_name(nlp)])  # type: ignore
        clf.link_evidence(doc)  # type: ignore
    ...
    def test_link_evidence_clf(self):
        text = "Opacity most likely due to atelectasis."
        doc = nlp(text, disable=[get_document_classifier_pipe_name(nlp)])  # type: ignore
        cls1 = clf.classify_document(doc, schema="linked")  # type: ignore
        assert cls1 == "NEG"
    ...
    def test_link_evidence_pos_clf(self):
        ...
        cls1 = clf.classify_document(doc, classification_schema="linked")  # type: ignore
        assert cls1 == "POS"

    def test_not_link_evidence_clf(self):
        text = "Opacity most likely due to atelectasis. There is infiltrate."
        doc = nlp(text, disable=[get_document_classifier_pipe_name(nlp)])  # type: ignore
        cls1 = clf.classify_document(doc, classification_schema="full")  # type: ignore
        assert cls1 == "NEG"
